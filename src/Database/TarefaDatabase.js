...
export default class ItemDatabase {
                ...
                SQLite.openDatabase(database_name, database_version, database_displayname, database_size).then(DB => {
                    ...
                    db.executeSql('SELECT 1 FROM Tarefa LIMIT 1').then(() => {
                        ...
                        db.transaction((tx) => {
                            ...
                            tx.executeSql('CREATE TABLE IF NOT EXISTS Tarefa (id INTEGER PRIMARY KEY AUTOINCREMENT, prazo INTEGER, prioridade varchar(30), descricao varchar(30), status varchar(30))');
                        }).then(() => {
                ...
                db.transaction((tx) => {
                    ...
                    tx.executeSql('SELECT * FROM Tarefa', []).then(([tx, results]) => {
                        ...
                        for (let i = 0; i < len; i++) {
                            ...
                            const { id, prazo, prioridade, descricao, status } = row;
                            lista.push({ id, prazo, prioridade, descricao, status });
                        }
                        console.log(lista);
                        resolve(lista);
                    });
        ...
        return new Promise((resolve) => {
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    ...
                    tx.executeSql('INSERT INTO Tarefa (prazo, prioridade, descricao, status) VALUES (?, ?, ?, ?)', [tarefa.prazo, tarefa.prioridade, tarefa.descricao, tarefa.status]).then(([tx, results]) => {
                        resolve(results);
                    });
                }).then((result) => {
                    this.Desconectar(db);
    ...
    Atualizar(status , id) {   //**** CRUD => UPDATE - aqui a tabela é atualizada *****/
        return new Promise((resolve) => {
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    //Query SQL para atualizar um registro no banco        
                    tx.executeSql('UPDATE Tarefa SET status = ? WHERE id = ?', [status, id]).then(([tx, results]) => {
                        resolve(results);
                    });
                }).then((result) => {
                    this.Desconectar(db);
